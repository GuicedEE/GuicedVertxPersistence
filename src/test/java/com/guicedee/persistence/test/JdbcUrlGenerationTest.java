...
public class JdbcUrlGenerationTest {
    ...
    public void testCustomPropertiesInJdbcUrl() {
        ...
        // Verify the URL with custom properties (order-independent check)
        String expectedPrefix = "jdbc:postgresql://localhost:5432/testdb?";
        assertEquals(0, jdbcUrl.indexOf(expectedPrefix), "URL should start with " + expectedPrefix);
        String queryPart = jdbcUrl.substring(expectedPrefix.length());
        String[] params = queryPart.split("&");
        java.util.Set<String> paramSet = new java.util.HashSet<>(java.util.Arrays.asList(params));
        assertEquals(2, paramSet.size(), "Should have 2 parameters");
        org.junit.jupiter.api.Assertions.assertTrue(paramSet.contains("ssl=true"), "Should contain ssl=true");
        org.junit.jupiter.api.Assertions.assertTrue(paramSet.contains("sslmode=require"), "Should contain sslmode=require");
    }
