...
The GuicedEE Persistence library (`com.guicedee.persistence`) is a lightweight, Guice-friendly bridge that binds persistence units defined in `persistence.xml` to Hibernate Reactive 7, Mutiny, and Vert.x 5. The diagram below captures the system boundary and the primary actors/tech stacks we depend on.
...
C4Context
    ...
    System_Boundary(persistence, "GuicedEE Persistence Library (com.guicedee.persistence)") {
...
- **Database edge:** TLS enforcement and driver provenance (use `com.guicedee.modules.services` artifacts) protect the RDBMS boundary; reject non-approved drivers.  
